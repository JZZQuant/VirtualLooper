...
class RecordState(State):
    ...
    def on_control(self, value, timestamp, time_delta, midi):
        if time_delta < self.long_press_time and self.session.control_on:
            ...
            max_size = max([len(layer.data) for layer in self.session.active_phrase.layers] + [
                len(self.session.active_phrase.phrase.data)])
            ...
            self.session.active_phrase.overdub.post_padding(
                postpend_length * [np.zeros(2 * self.session.frames_per_buffer)])
